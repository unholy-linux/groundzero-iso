...
iso_name=GZLx
iso_label="GZLx-v1.8.1-$(date +%Y%m)-x86_64"
iso_version="v1.8.1-$(date +%Y.%m.%d)"
install_dir=arch
work_dir=work
out_dir=out
gpg_key=
...
make_packages() {
    if [[ ${arch} == "x86_64" ]]; then
          ...
          setarch ${arch} mkarchiso ${verbose} -w "${work_dir}/${arch}" -C "${work_dir}/pacman.conf" -D "${install_dir}" -r "pacman -Rdd --noconfirm gcc-libs" run
    fi
